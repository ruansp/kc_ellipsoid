function Ellipse_plot(A, C, color, dashed, varargin)
% 
...
if strcmp(Type,'2D')
    if strcmp(dashed, 'on')
        ...
        % Added by Qianli Ma
        if color == 'r'
            h1 = patch(X(1,:),X(2,:), color);
        elseif color == 'y'
            h2 = patch(X(1,:),X(2,:), color);
        else
        end
    end
    hold on;
...
%    mesh(XX,YY,ZZ,'EdgeColor','red');
    if strcmp(color, 'blue')
        mesh(XX,YY,ZZ,'EdgeColor',color);
    else
        surf(XX,YY,ZZ,'FaceColor', color);
        alpha(.3)
    end
    axis equal
    hidden off
